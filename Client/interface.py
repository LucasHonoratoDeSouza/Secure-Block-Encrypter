...
class App(QWidget):
    ...
    def retrieve_data(self):
        key = self.retrieve_key_entry.text()
        block_hash = self.retrieve_block_hash_entry.text()
        if key and block_hash:
            response = requests.get(f'{self.server_url}/get_data/{block_hash}', params={'key': key.encode()})
            ...
            QMessageBox.critical(self, "Error", "Please enter both key and block hash.")
